...
const CryptoChartCard = ({ id, name }) => {
  ...
  const getCoinMarketData = () => {
    ...
    .then((res) => setCoinData(res.data.market_data))
    .catch((e) => console.log(e))
  }

  const getCoinChartData = () => {
    axios.get(`https://api.coingecko.com/api/v3/coins/${name}/market_chart?vs_currency=usd&days=1`)
    .then((res) => setOptions((prevOptions) => {
      return { ...prevOptions, series: [{ data: res.data.prices }] }
    }))
    .catch((e) => console.log(e))
  }
